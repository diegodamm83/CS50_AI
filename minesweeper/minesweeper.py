...
class MinesweeperAI():
    ...
    def get_neighbors(self, cell):
        '''
        Returns the neighbors of a cell and thier mine count
        '''
        i, j = cell
        neighbors = set()
    ...
    def add_knowledge(self, cell, count):
        ...
        for followingSentence in self.knowledge:
            if len(followingSentence.cells) == 0:
                self.knowledge.remove(followingSentence)
            elif sentence == followingSentence:
                break
            elif sentence.cells <= followingSentence.cells:
                unique_set = followingSentence.cells - sentence.cells
                diff_num = followingSentence.count - sentence.count
    ...
    def make_random_move(self):
        ...
        while True:
            ...
            if (i, j) not in self.moves_made and (i, j) not in self.mines and (i, j) not in moves_explored:
                return (i, j)
            elif not (len(moves_explored.union(self.moves_made))+len(self.mines) == self.height*self.width):
                moves_explored.add((i, j))
            else:
                return None
